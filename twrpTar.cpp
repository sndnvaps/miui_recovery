...
int twrpTar::createTarFork() {
	...
	//	}
		...
		if (utils::Wait_For_Child(pid, &status, "createTarFork()") != 0)
			return -1;
	}
	...
	if (pid >= 0) // fork was successful
	{
		if (pid == 0) // child process
		{
			if (utils::Path_Exists(tarfn)) {
				...
				if (utils::Get_File_Type(tarfn) != 2) {
				...
				for (i = start_thread_id; i < 9; i++) {
					...
					if (utils::Path_Exists(actual_filename)) {
						thread_count++;
		...
		{
			if (utils::Wait_For_Child(pid, &status, "extractTarFork()") != 0)
				return -1;
		}
	}
...
int twrpTar::splitArchiveFork() {
	...
	if (pid >= 0) // fork was successful
	{
		...
		{
			if (utils::Wait_For_Child(pid, &status, "splitArchiveFork()") != 0)
				return -1;
		}
	}
	else // fork has failed
...
int twrpTar::Split_Archive()
{
	...
	if (!tarexclude.empty())
		split = utils::split_string(tarsplit, ' ', true);
	createTar();
	//DataManager::GetValue(TW_HAS_DATA_MEDIA, has_data_media);
	utils::Is_Data_Media(has_data_media);
...
int twrpTar::extract() {
	Archive_Current_Type = utils::Get_File_Type(tarfn);
	...
	} else if (Archive_Current_Type != 2 )  { // type = 2, is encrypted tar files
...
void* twrpTar::extractMulti(void *cookie) {
	...
	while (utils::Path_Exists(actual_filename)) {
...
int twrpTar::closeTar() {
	...
	if (Archive_Current_Type > 0) {
		close(fd);
		int status;
		if (pigz_pid > 0 && utils::Wait_For_Child(pigz_pid, &status, "pigz") != 0)
			return -1;
		/*
		 * This is not need for miui recovery 
		if (oaes_pid > 0 && utils::Wait_For_Child(oaes_pid, &status, "openaes") != 0)
			return -1;
		*/
	}
	free_libtar_buffer();
...
unsigned long long twrpTar::uncompressedSize() {
	...
	Tar = utils::Get_Filename(tarfn);
	type = utils::Get_File_Type(tarfn);
	if (type == 0)
		total_size = utils::Get_File_Size(tarfn);
	else {
		Command = "pigz -l " + tarfn;
		/* if we set Command = "pigz -l " + tarfn + " | sed '1d' | cut -f5 -d' '";
		   we get the uncompressed size at once. */
		utils::Exec_Cmd(Command, result);
		if (!result.empty()) {
			/* Expected output:
				compressed   original reduced  name
				  95855838  179403776   -1.3%  data.yaffs2.win
						^
					     split[5]
			*/
			split = utils::split_string(result, ' ', true);
			if (split.size() > 4)
